...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
 ...
 The store is an application-level state that is available to all components. Every time a change is made to a property in the store, the change is immediately reflected in every component that is connected to the store. The store cannot be changed by any component, only replaced by special functions called reducers, which receive as an input an action 'type' and an associated 'payload' (or piece of data) from an action function. 
 
 The action function tells the reducer function how and with what the store is to be updated.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is an overriding state that is available to all components at the same time. Component state is a state that is only availble to an individual component and is not shared with or accessible by other components. 

A good time to use application level state is when getting an initial data set. Component level state is useful when toggling a view within a component or passing a new object to an action creator which will then add it to the application level state.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux thunk is a middleware package that allows us to call functions in our action creators instead of just passing a type and payload. Calling functions in our middlewear lets us perform asychronious data operations. 
